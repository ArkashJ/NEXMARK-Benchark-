...
public class Query8 {
    ...
    public static void main(String[] args) throws Exception {
        ...
        env.enableCheckpointing(params.getLong("interval"), CheckpointingMode.EXACTLY_ONCE);
        ...
        env.setParallelism(params.getInt("p-join"));      //set parallelism for join


        KafkaSource<Person> personKafkaSource = KafkaSource.<Person>builder()
                .setBootstrapServers(Broker)
                .setGroupId(personKafkaGroup)
                .setTopics(personKafkaTopic)
                .setDeserializer(KafkaRecordDeserializationSchema.valueOnly(new PersonSchema()))
                .setProperty(KafkaSourceOptions.REGISTER_KAFKA_CONSUMER_METRICS.key(), "true")
                .setStartingOffsets(OffsetsInitializer.earliest())
                .build();
        DataStream<Person> persons = env.fromSource(personKafkaSource, WatermarkStrategy.noWatermarks(), "Person Kafka source")
                .setParallelism(1)
                .slotSharingGroup("person-src");
        DataStream<Person> personsWithWaterMark = persons.assignTimestampsAndWatermarks(new PersonTimestampAssigner())
                .setParallelism(1)
                .slotSharingGroup("person-timestamp");

        KafkaSource<Auction> auctionKafkaSource = KafkaSource.<Auction>builder()
                .setBootstrapServers(Broker)
                .setGroupId(auctionKafkaGroup)
                .setTopics(auctionKafkaTopic)
                .setDeserializer(KafkaRecordDeserializationSchema.valueOnly(new AuctionSchema()))
                .setProperty(KafkaSourceOptions.REGISTER_KAFKA_CONSUMER_METRICS.key(), "true")
                .setStartingOffsets(OffsetsInitializer.earliest())
                .build();
        DataStream<Auction> auctions = env.fromSource(auctionKafkaSource, WatermarkStrategy.noWatermarks(), "Auction Kafka source")
                .setParallelism(1)
                .slotSharingGroup("auc-src");
        DataStream<Auction> auctionsWithWaterMark = auctions.assignTimestampsAndWatermarks(new AuctionTimestampAssigner())
                .setParallelism(1)
